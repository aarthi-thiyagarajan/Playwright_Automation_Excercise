...
- Location: tests\AEQ-11_user_signup.spec.ts:227:7
...
Locator: locator('p:text("Email Address already exist!")')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('p:text("Email Address already exist!")')
...
- text: Mrs. Name
- superscript: "*"
- textbox "Name *": Case Test User
- text: Email
- superscript: "*"
- textbox "Email *" [disabled]: PRE.REG.1785568307587@MAILTEST.COM
- text: Password
- superscript: "*"
- textbox "Password *"
- text: Date of Birth
...
- insertion:
  ...
  - link "TV & Video"
  - link "Test case creation"
- insertion:
  - iframe
- button "Privacy and cookie settings"
- text: "Managed by Google. Complies with IAB TCF. CMP ID: 300"
- img
- link "Go to shopping options for Test case creation": Test case creation
...
```ts
  ...
  17 |     await expect(this.page.getByRole('heading', { name: 'New User Signup!' })).toBeVisible();
  18 |   }
  19 | 
  20 |   async enterName(name: string): Promise<void> {
  21 |     await this.nameInput.fill(name);
  22 |   }
  23 | 
  24 |   async enterEmail(email: string): Promise<void> {
  25 |     await this.emailInput.fill(email);
  26 |   }
  27 | 
  28 |   async clearName(): Promise<void> {
  29 |     await this.nameInput.clear();
  30 |   }
  31 | 
  32 |   async clearEmail(): Promise<void> {
  33 |     await this.emailInput.clear();
  34 |   }
  35 | 
  36 |   async clickSignupButton(): Promise<void> {
  37 |     await this.signupButton.click();
  38 |   }
  39 | 
  40 |   async getDuplicateEmailError(): Promise<string> {
> 41 |     await expect(this.duplicateEmailError).toBeVisible();
     |                                            ^ Error: expect(locator).toBeVisible() failed
  42 |     return this.duplicateEmailError.innerText();
  43 |   }
  44 | 
  45 |   async fillSignupForm(name: string, email: string): Promise<void> {
  46 |     await this.enterName(name);
  47 |     await this.enterEmail(email);
  48 |     await this.clickSignupButton();
  49 |   }
  50 | }
  51 | 
```